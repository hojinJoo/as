...
if __name__ =="__main__" :
    ...
    sample = a[40]
    vocal = sample['vocals']
    accom = torch.sum(torch.stack([sample[source] for source in ['drums', 'bass', 'other']], dim=0), dim=0)
    
    mix = vocal + accom
    
    audio_signal = nussl.AudioSignal(audio_data_array=torch_isfft(torch.abs(torch.view_as_real(mix)),window_length=1024,nfft=4096,hop_length=1024,original_length=44100).numpy(),sample_rate=44100)
    ...
    ensemble = nussl.separation.composite.EnsembleClustering(
        audio_signal, 6, separators=separators, 
        fit_clusterer=True, weights=weights, returns=returns,extracted_feature='estimates')
    feat = ensemble.extract_features()
    res = ensemble.cluster_features(feat,ensemble.clusterer)
    print(res.shape)
    # estimates,res = ensemble.run()
    print(f"ensembble clusterer center point {np.array(ensemble.clusterer.cluster_centers_).shape}")
